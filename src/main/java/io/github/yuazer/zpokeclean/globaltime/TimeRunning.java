...
//            }
//        }
        String t = String.valueOf(YamlUtils.getConfigInt("GlobalSetting.TimeSetting") * 60 - getGlobalTime());
        if (Main.getInstance().getTimeMsg().contains(t)) {
            Bukkit.getServer().broadcastMessage(YamlUtils.getConfigMessage("ClearMessage." + t));
        }
    }
}
